...
def mapCb(msg):
    ...
    if (pose):
        pose = pose.pose.pose
        rospy.set_param('sesto0/amcl/initial_pose_x', pose.position.x)
        rospy.set_param('sesto0/amcl/initial_pose_y', pose.position.y)
        roll, pitch, yaw = tf.transformations.euler_from_quaternion( \
            [pose.orientation.x, pose.orientation.y, pose.orientation.z, pose.orientation.w])
        rospy.set_param('sesto0/amcl/initial_pose_a', yaw)
...
if (len(sys.argv) < 2):
    ...
    proc = subprocess.Popen(["roslaunch", sys.argv[1]])
    secondary = rospy.Subscriber("/sesto0/map", OccupancyGrid, mapCb)
    pose_out = rospy.Publisher("/sesto0/amclinitialpose", PoseWithCovarianceStamped)
    sub = rospy.Subscriber("/sesto0/initialpose", PoseWithCovarianceStamped, poseCb)
    listener = tf.TransformListener(True, rospy.Duration(100))
    bad_counter = 0
    while not rospy.is_shutdown():
        try:
            (trans,rot) = listener.lookupTransform('sesto0/map', 'sesto0/sesto_frame', rospy.Time())
            bad_counter = 0
